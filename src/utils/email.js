...
const sendEmail = async (to, subject, text, html = null) => {
  try {
    ...
    const fromName = settings?.emailFromName || process.env.EMAIL_FROM_NAME || 'Kidsworldbd';
...
const sendOTPEmail = async (email, otp) => {
  ...
  const subject = 'Your Kidsworldbd Verification Code';
  const text = `Your Kidsworldbd verification code is: ${otp}. This code will expire in 5 minutes. Please do not share this code with anyone.`;
            ...
            <table role="presentation" style="width: 600px; border-collapse: collapse; background-color: #ffffff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); overflow: hidden;">
              ...
              <tr>
                <td style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); padding: 40px 40px 30px; text-align: center;">
                  <h1 style="margin: 0; color: #ffffff; font-size: 28px; font-weight: 600; letter-spacing: -0.5px;">Kidsworldbd</h1>
                  <p style="margin: 8px 0 0; color: #dbeafe; font-size: 14px; font-weight: 400;">Your trusted shopping partner</p>
                </td>
              </tr>
              ...
              <tr>
                <td style="padding: 40px;">
                  ...
                  <p style="margin: 0 0 24px; color: #4b5563; font-size: 15px; line-height: 1.6;">
                    Thank you for registering with Kidsworldbd! Your register OTP is:
                  </p>
                  
                  <!-- OTP Box -->
                  <div style="background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); border: 2px solid #bfdbfe; border-radius: 12px; padding: 30px; text-align: center; margin: 30px 0;">
                    <p style="margin: 0 0 12px; color: #1e40af; font-size: 13px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;">Your Verification Code</p>
                    <div style="font-size: 36px; font-weight: 700; letter-spacing: 8px; color: #1d4ed8; font-family: 'Courier New', monospace; margin: 8px 0;">
                      ${otp}
                    </div>
                  </div>
                  
                  <p style="margin: 24px 0 0; color: #6b7280; font-size: 14px; line-height: 1.6;">
                    This code will expire in <strong style="color: #1d4ed8;">5 minutes</strong>. Please use it to complete your registration.
                  </p>
                  
                  <p style="margin: 20px 0 0; color: #9ca3af; font-size: 13px; line-height: 1.6;">
                    <strong style="color: #6b7280;">⚠️ Security Notice:</strong> Never share this code with anyone. Kidsworldbd will never ask for your OTP.
                  </p>
                </td>
              </tr>
              ...
              <tr>
                <td style="background-color: #f9fafb; padding: 30px 40px; text-align: center; border-top: 1px solid #e5e7eb;">
                  ...
                  <p style="margin: 0; color: #9ca3af; font-size: 12px;">
                    © ${new Date().getFullYear()} Kidsworldbd. All rights reserved.
                  </p>
                </td>
              </tr>
            </table>
              ...
              <table role="presentation" style="width: 600px; border-collapse: collapse; background-color: #ffffff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); overflow: hidden;">
                ...
                <tr>
                  <td style="padding: 40px;">
                    ...
                    <!-- Order Info Box -->
                    <div style="background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); border: 2px solid #bfdbfe; border-radius: 12px; padding: 24px; margin: 24px 0;">
                      <p style="margin: 0 0 8px; color: #1f2937; font-size: 14px; line-height: 1.6;">
                        <span style="font-weight: 600; color: #1e40af;">Order Number:</span> 
                        <span style="color: #1d4ed8; font-weight: 700; font-family: 'Courier New', monospace;">#${order.orderId}</span>
                      </p>
                      <p style="margin: 0 0 8px; color: #1f2937; font-size: 14px; line-height: 1.6;">
                        <span style="font-weight: 600; color: #1e40af;">Order Date:</span> 
                        <span style="color: #1d4ed8; font-weight: 600;">${orderDate}</span>
                      </p>
                      <p style="margin: 0 0 8px; color: #1f2937; font-size: 14px; line-height: 1.6;">
                        <span style="font-weight: 600; color: #1e40af;">Status:</span> 
                        <span style="color: #1d4ed8; font-weight: 600;">${statusDisplay}</span>
                      </p>
                      <p style="margin: 0; color: #1f2937; font-size: 14px; line-height: 1.6;">
                        <span style="font-weight: 600; color: #1e40af;">Payment Method:</span> 
                        <span style="color: #1d4ed8; font-weight: 600;">${paymentMethodDisplay}</span>
                      </p>
                    </div>
                    ...
                    <div style="background-color: #f9fafb; border-radius: 8px; padding: 20px; margin: 24px 0;">
                      ...
                      <table role="presentation" style="width: 100%; border-collapse: collapse;">
                        ...
                        <tr style="border-top: 2px solid #e5e7eb;">
                          ...
                          <td style="padding: 12px 0 0; text-align: right; color: #1d4ed8; font-size: 18px; font-weight: 700;">৳${finalTotal.toFixed(2)}</td>
                        </tr>
                      </table>
                    </div>
                    ...
                    <div style="text-align: center; margin: 32px 0 0;">
                      <a href="${orderDetailsUrl}" style="display: inline-block; background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 15px;">View Order Details</a>
                    </div>
...
const sendWelcomeEmail = async (user, signupBonusCoins = 0) => {
  try {
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
    const subject = 'Welcome to Kidsworldbd! 🎉';
    const text = `Welcome to Kidsworldbd, ${user.name}! Thank you for joining us. We're excited to have you as part of our community.${signupBonusCoins > 0 ? ` As a welcome gift, you've received ${signupBonusCoins} coins!` : ''} Start shopping now: ${frontendUrl}`;
          ...
          <tr>
            <td align="center">
              <table role="presentation" style="width: 600px; border-collapse: collapse; background-color: #ffffff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); overflow: hidden;">
                ...
                <tr>
                  <td style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); padding: 40px 40px 30px; text-align: center;">
                    <h1 style="margin: 0; color: #ffffff; font-size: 28px; font-weight: 600; letter-spacing: -0.5px;">Welcome to Kidsworldbd! 🎉</h1>
                    <p style="margin: 8px 0 0; color: #dbeafe; font-size: 14px; font-weight: 400;">Your trusted shopping partner</p>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="padding: 40px;">
                    ...
                    <p style="margin: 0 0 24px; color: #4b5563; font-size: 15px; line-height: 1.6;">
                      Thank you for joining Kidsworldbd! We're thrilled to have you as part of our community. Your account has been successfully created and you're all set to start shopping.
                    </p>
                    
                    ${signupBonusCoins > 0 ? `
                    <!-- Welcome Bonus Box -->
                    <div style="background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); border: 2px solid #bfdbfe; border-radius: 12px; padding: 24px; margin: 24px 0; text-align: center;">
                      <p style="margin: 0 0 12px; color: #1e40af; font-size: 13px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;">🎁 Welcome Bonus</p>
                      <p style="margin: 0; color: #1d4ed8; font-size: 18px; font-weight: 700;">
                    ...
                    <!-- CTA Button -->
                    <div style="text-align: center; margin: 32px 0 0;">
                      <a href="${frontendUrl}" style="display: inline-block; background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 15px;">Start Shopping Now</a>
                    </div>
                  </td>
                </tr>
