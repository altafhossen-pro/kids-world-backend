...
// Add toJSON transform to inject displayTotalSold dynamically
productSchema.set('toJSON', {
  virtuals: true,
  transform: function (doc, ret) {
    if (ret.salesAnalytics && ret.salesAnalytics.isActive && ret.salesAnalytics.startDate) {
      const daysPassed = Math.floor((Date.now() - new Date(ret.salesAnalytics.startDate).getTime()) / (1000 * 60 * 60 * 24));
      if (daysPassed >= 0) {
        const seed = parseInt(ret._id.toString().slice(-4), 16) || 0;
        const variance = Math.floor(Math.sin(daysPassed + seed) * (ret.salesAnalytics.dailyAverage / 2));
        ret.displayTotalSold = (ret.totalSold || 0) + (daysPassed * ret.salesAnalytics.dailyAverage) + variance;
      } else {
        ret.displayTotalSold = ret.totalSold || 0;
      }
    } else {
      ret.displayTotalSold = ret.totalSold || 0;
    }
