      ...
      try {
        ...
        if (settings && settings.isSendUserOrderSMS !== false) {
          ...
          if (userPhone) {
            ...
            const smsMessage = `Kidsworldbd: Order #${order.orderId} confirmed. Total: ${totalAmount} Tk. Track: ${trackingUrl}`;
    ...
    if (updates.status === 'confirmed' && oldOrder.status !== 'confirmed') {
      ...
      try {
        ...
        if (userPhone && settings && settings.isSendOrderStatusConfirmedSMS !== false) {
          ...
          const trackingUrl = `${frontendUrl}/tracking?orderId=${order.orderId}`;
          const totalAmount = order.total.toFixed(2);
          const smsMessage = `Kidsworldbd: Your order #${order.orderId} has been Confirmed. Total: ${totalAmount} Tk. Track: ${trackingUrl}`;
    ...
    if (updateData.status === 'confirmed' && oldOrder.status !== 'confirmed') {
      try {
        ...
        if (userPhone && settings && settings.isSendOrderStatusConfirmedSMS !== false) {
          ...
          const smsMessage = `Kidsworldbd: Your order #${updatedOrder.orderId} has been Confirmed. Total: ${totalAmount} Tk. Track: ${trackingUrl}`;
          sendCustomSMS(userPhone, smsMessage).catch(err => {
            console.error('Failed to send order confirmed SMS:', err);
          });
        }
